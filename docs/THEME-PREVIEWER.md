...
see [`THEME-FORMAT.md`](THEME-FORMAT.md), which also records the
[14 places](THEME-FORMAT.md#10-documentation-discrepancies) where the official
theme guide and the code disagree.

The point of it is the feedback loop. A theme is a text file whose errors are
silent: a missing key does not fail, it draws nothing; a colour without a `#`
does not error, it comes out black; an oversized cover cache does not warn, it
black-screens a console that a PC emulator renders perfectly. All of that is
visible here before it reaches hardware.
...
resident — the element's drawn size does not reduce it. Shared patterns are
counted once, matching OPL's `findDuplicate`. A 512×512 cover with `count=40` is
~31 MB against a 32 MB console — a theme that looks perfect in PCSX2 and
black-screens on real hardware.

Every diagnostic is clickable and jumps to the offending line.
...
The previewer implements the behaviour added by
[the patches](../patches/README.md) — grid mode on `ItemsList`, `offset` on
`GameImage`, `MenuTabs`, `RecentImage`/`RecentText`, `x_scaled`, and the rest —
and flags every patch-only key with a `PATCHED_ONLY` note.

That flag is the point: a theme that uses them is not portable to a stock OPL
build, and without the marker that is invisible until someone else's console
draws nothing.
...
`assets/PoeVeticaNew.ttf` is OPL's built-in font, vendored from the
Open-PS2-Loader tree (`thirdparty/`) and carrying its own licence. It is the
source of truth for the base64 copy embedded in the HTML, which is what makes
text metrics correct over `file://`.

Regenerate the embed after changing the font:
