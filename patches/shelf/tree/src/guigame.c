...
/* Cheats have a global tier and always did -- cheatman.c reads $EnableCheat and
   $CheatMode from conf_game.cfg for any game whose own config does not claim
   them. What it never had was a door of its own: the only way in was to
   highlight some arbitrary game, press triangle, and set a dropdown in a menu
   labelled per-game that then wrote a different file. This is the flag that
   lets the same dialog serve the main menu, exactly as forceGlobalOSDLanguage
   already does for OSD Language -- one dialog, two entrances, no second copy of
   the settings to disagree with the first. */
static int forceGlobalCheat;
...
static void guiGameSetCheatSettingsState(void)
{
    ...
    if (forceGlobalCheat) {
        /* No game to scope to, so the source row is inert and the only live
           controls are the two values themselves. */
...
/** The cheat dialog, from either entrance.
 *
 *  forceGlobal is what the main menu passes. It pins Settings Source to Global
 *  and greys it out -- greyed rather than hidden, so the row still says which
 *  tier you are editing instead of leaving you to infer it -- and reads the
 *  values from conf_game.cfg alone.
 *
 *  Nothing about the two tiers changed. A game that has claimed $CheatsSource
 *  for itself still wins, and still has to be edited from its own menu; this
 *  only means the tier that governs everything else is reachable without
 *  picking a game first. */
void guiGameShowCheatConfig(int forceGlobal)
{
    // configure the enumerations
    const char *settingsSource[] = {_l(_STR_GLOBAL_SETTINGS), _l(_STR_PERGAME_SETTINGS), NULL};
    const char *cheatmodeNames[] = {_l(_STR_CHEATMODEAUTO), _l(_STR_CHEATMODESELECT), NULL};

    forceGlobalCheat = forceGlobal;

    diaSetEnum(diaCheatConfig, CHTCFG_CHEATSOURCE, settingsSource);
    diaSetEnum(diaCheatConfig, CHTCFG_CHEATMODE, cheatmodeNames);
    diaSetEnabled(diaCheatConfig, CHTCFG_CHEATSOURCE, !forceGlobal);

    if (forceGlobal)
        guiGameLoadCheatsConfig(NULL, configGetByType(CONFIG_GAME));
...
/** Persist the global tier. Called from the main menu's Save Changes, beside
 *  guiGameSaveOSDLanguageGlobalConfig, which is where every other global
 *  game-behaviour setting is written -- the shared saveConfig call there
 *  already covers CONFIG_GAME. */
void guiGameSaveCheatsGlobalConfig(config_set_t *configGame)
{
    /* Scope-guarded, exactly as guiGameSaveOSDLanguageGlobalConfig is. EnableCheat
       and CheatMode are file statics shared by both entrances, so without this a
       visit to a game that keeps its own cheat settings would leave per-game
       values in them and Save Changes would copy that game's preferences into
       everybody's. gCheatSource records which tier last filled them. */
...
static void guiGameLoadCheatsConfig(config_set_t *configSet, config_set_t *configGame)
{
    ...
    // override global with per-game settings if available and selected.
    /* Guarded, because opened from the main menu there is no game and configSet
       is NULL -- getConfigItemForName would walk a null list. Same guard the
       OSD Language dialog uses for the same reason. */
    if (!forceGlobalCheat) {
