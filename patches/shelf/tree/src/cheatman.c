...
int load_cheats(const char *cheatfile)
{
    ...
    if (buf == NULL) {
        /* Tell "there is no cheat file" apart from "there is one and it would
           not load". Both used to return -1, and every caller switches on
           -ENOENT to choose between "No cheats found" and "Cheats loading
           failed" -- so the -ENOENT arm was dead code and every game without a
           cheat file reported a failure instead. On a library where 18 of 46
           have no patch that is a scary message for the normal case. */
        int fd = open(cheatfile, O_RDONLY);
        if (fd < 0) {
            LOG("%s: No cheats file '%s'\n", __FUNCTION__, cheatfile);
            return -ENOENT;
        }
        close(fd);
