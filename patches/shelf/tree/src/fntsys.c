...
int fntRenderString(int id, int x, int y, short aligned, size_t width, size_t height, const char *string, u64 colour)
{
    ...
    if (aligned & ALIGN_HCENTER) {
        ...
        /* Subtracted here, after x is in physical pixels, because
           fntCalcDimensions is physical too. A caller doing this in virtual
           space gets an error proportional to the string's own length. */
