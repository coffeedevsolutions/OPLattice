...
static void menuRenderElements(theme_element_t *elem)
{
    ...
    while (elem) {
        if (elem->drawElem) {
            /* x_scaled elements track the widescreen narrowing. Done here rather
             * than in each draw function, and restored afterwards so the stored
             * position stays the one the theme wrote. */
            int savedX = elem->posX;

            if (elem->xScaled) {
                int centre = 640 >> 1;
                elem->posX = centre + rmWideScale(elem->posX - centre);
            }
...
{
    /* A grid needs both d-pad axes for the tiles, so device switching moves to
     * the shoulders: L1/R1 change device, L2/R2 page through the list. Classic
     * single-column themes keep stock OPL's mapping exactly.
     */
    ...
    } else if (getKey(KEY_L1)) { // page up, or previous device in a grid theme
        if (grid)
            menuPrevH();
        else
            menuPrevPage();
    } else if (getKey(KEY_R1)) { // page down, or next device in a grid theme
        if (grid)
            menuNextH();
        else
            menuNextPage();
    } else if (getKeyOn(KEY_L2)) { // home, or page up in a grid theme
        if (grid)
            menuPrevPage();
        else
            menuFirstPage();
    } else if (getKeyOn(KEY_R2)) { // end, or page down in a grid theme
        if (grid)
            menuNextPage();
        else
            menuLastPage();
